...
def main():
    ...
    if args.list:
        ...
        # for rd, unique_name in zip(run_dirs, ["latency-aligned (SSP)", "latency-misaligned (SSP)", "latency-aligned (MSP)", "latency-misaligned (MSP)"]):
        for rd, unique_name in zip(run_dirs, ["$c=0$ (SSP)", "$c=0.001$ (SSP)", "$c=0.002$ (SSP)", "$c=0.003$ (SSP)", "$c=0$ (MSP)", "$c=0.001$ (MSP)", "$c=0.002$ (MSP)", "$c=0.003$ (MSP)"]):
            try:
                print(f"\nProcessing {rd} ...")
        ...
        axes = axes.flatten()
        # for idx, y, y_label in zip(range(3), ["gini", "liveness", "hhi"], ["$Gini_{g}$",  "$LC_{g}$","$HHI_{g}$"]):
