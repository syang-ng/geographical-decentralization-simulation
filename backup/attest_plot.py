...
def single_line(ax: plt.Axes, data_df: pd.DataFrame, x_col: str, y_col: str, hue: str, ylabel: str, title: str = "", legend: bool = True, xlabel: str = "Slot"):
    sns.lineplot(
        ...
        hue=hue,
        style=hue,
        dashes=[(None, None), (None, None), (None, None), (None, None), (1,1), (1,1),(1,1), (1,1)],
        ax=ax,
        lw=8.0,
        legend=legend
    )
...
def main():
    ...
    if args.list:
        ...
        # for rd, unique_name in zip(run_dirs, ["latency-aligned (SSP)", "latency-misaligned (SSP)", "latency-aligned (MSP)", "latency-misaligned (MSP)"]):
        for rd, unique_name in zip(run_dirs, [r"$\gamma=1/3$ (SSP)", r"$\gamma=1/2$ (SSP)", r"$\gamma=2/3$ (SSP)", r"$\gamma=4/5$ (SSP)", r"$\gamma=1/3$ (MSP)", r"$\gamma=1/2$ (MSP)", r"$\gamma=2/3$ (MSP)", r"$\gamma=4/5$ (MSP)"]):
            try:
                print(f"\nProcessing {rd} ...")
        ...
        axes = axes.flatten()
        for idx, y, y_label in zip(range(3), ["gini", "liveness", "hhi"], ["Gini$_{g}$", "HHI$_{g}$", "LC$_{g}$"]):
            ax = axes[idx]
            single_line(
                axes[idx],
        ...
        fig.legend(handles, labels, loc="upper center", fontsize=40, ncol=4, title=None, bbox_to_anchor=(0.5, 1.04), 
            framealpha=0, facecolor="none", edgecolor="none", columnspacing=0.5)
